...
const industries: [string, string, Icon][] = [
  ["食品與農產加工", "製程溫壓、濃度、環境與批次品質監控", Gauge],
  ["紡織・製衣與鞋材", "色差、幅寬、瑕疵與生產狀態檢測", Swatches],
  ["電子與能源", "生產履歷、光學檢測與跨場域電力管理", Scan],
  ["金屬與表面處理", "槽液溫度、電流與關鍵製程參數監控", Drop],
  ["材料・薄膜與化工", "製程條件、設備狀態與異常趨勢追蹤", BezierCurve],
  ["檢驗與品保單位", "檢測數據整合、品質驗證與報表追溯", FirstAidKit],
];
...
export default function App() {
  ...
  const caseControlRef = useRef<{ nudge: (direction: 1 | -1) => void } | null>(null);
  ...
  useEffect(() => {
    ...
    const originalCount = caseStudies.length;
    ...
    const updateActiveCard = () => {
      ...
      cards.forEach((card, index) => {
        ...
        if (distance < closestDistance) {
          ...
          closestIndex = index % originalCount;
        }
      });
    ...
    const wrapX = (loopWidth: number) => {
      const current = Number(gsap.getProperty(track, "x")) || 0;
      gsap.set(track, { x: gsap.utils.wrap(-loopWidth, 0, current) });
    };
    ...
    const setupSlider = () => {
      ...
      const loopWidth =
        originalCards.reduce((total, card) => total + card.offsetWidth, 0) +
      ...
      const alignToCard = (targetCard: HTMLElement) => {
        if (isSettling) {
          return;
        }
      ...
      tween = gsap.to(track, {
        ...
        modifiers: {
          x: gsap.utils.unitize((value) =>
            gsap.utils.wrap(-loopWidth, 0, parseFloat(value)),
          ),
        },
      });
      ...
      caseControlRef.current = {
        nudge: (direction) => {
          const currentCard = getClosestCard();
          const currentIndex = cards.indexOf(currentCard);
          const targetIndex = currentIndex + direction;
          const targetCard = cards[targetIndex] ?? (direction === 1 ? cards[0] : cards[cards.length - 1]);
          alignToCard(targetCard);
        },
      };
    };
  ...
  return (
    <main>
      <header className="site-header">
        <a className="brand" href="#top" aria-label="AEYEOT Datacanvas 首頁">
          <img className="brand-logo" src="/aeyeot-logo.svg" alt="AEYEOT Logo" />
        ...
        <nav className={menuOpen ? "nav-links is-open" : "nav-links"} aria-label="主要導覽">
          <a href="#capabilities" onClick={closeMenu}><StackSimple {...accentIconProps} className="nav-icon" />核心能力</a>
          <a href="#platform" onClick={closeMenu}><SquaresFour {...accentIconProps} className="nav-icon" />系統畫面</a>
          <a href="#cases" onClick={closeMenu}><Presentation {...accentIconProps} className="nav-icon" />執行案例</a>
          <a href="#scenarios" onClick={closeMenu}><Factory className="nav-icon" />應用情境</a>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-copy reveal">
          ...
          <div className="hero-actions">
            <a className="button button-primary" href="#contact">預約系統展示 <ArrowUpRight {...neutralIconProps} className="button-icon" /></a>
            <a className="button button-ghost" href="#capabilities">探索平台能力</a>
          </div>
          <div className="hero-proof">
            <span><PlugsConnected {...accentIconProps} className="proof-icon" /> 彈性串接既有系統</span>
            <span><RocketLaunch {...accentIconProps} className="proof-icon" /> 依需求規模快速導入</span>
          </div>
        </div>

        <div className="monitor-shell reveal reveal-delay" aria-label="Datacanvas 系統畫面示意">
          <img src="/cases/demo.png" alt="Datacanvas 系統畫面示意" />
        </div>
        <div className="hero-index" aria-hidden="true">DC / 001</div>
      </section>

      <section className="signal-strip" aria-label="平台價值">
        <span><Database {...accentIconProps} className="strip-icon" /> CONNECT <i /> 多源整合</span>
        <span><Pulse {...accentIconProps} className="strip-icon" /> OBSERVE <i /> 即時監控</span>
        <span><WarningCircle {...accentIconProps} className="strip-icon" /> DETECT <i /> 智慧判讀</span>
        <span><PaperPlaneTilt {...accentIconProps} className="strip-icon" /> RESPOND <i /> 主動應變</span>
      </section>
      ...
      <section className="problem section">
        ...
        <div className="gap-diagram" aria-label="從分散資訊到統一行動的流程">
          ...
          <div className="core-node"><img className="brand-logo" src="/aeyeot-logo.svg" alt="aeyeot logo"/><strong>Datacanvas</strong><small>ONE SOURCE OF TRUTH</small></div>
          <div className="flow-line outgoing"><i /><b>ACTIONABLE SIGNAL</b></div>
          <div className="action-stack">
            <span>即時告警 <WarningCircle {...accentIconProps} className="action-icon" /></span><span>責任通知 <PaperPlaneTilt {...accentIconProps} className="action-icon" /></span><span>持續改善 <ArrowUpRight {...accentIconProps} className="action-icon" /></span>
          </div>
        </div>
      </section>
        ...
        <div className="capability-grid">
          {capabilities.map((item) => (
            <article className="capability-card" key={item.index}>
              ...
              <div className="cap-icon" aria-hidden="true"><item.icon {...accentIconProps} /></div>
              <h3>{item.title}</h3>
              <p>{item.text}</p>
              <span className="card-corner"><ArrowUpRight {...accentIconProps} /></span>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="workflow section" id="workflow">
        <div className="workflow-copy">
          ...
          <a className="text-link" href="#contact">了解如何導入 <ArrowRight {...accentIconProps} className="text-link-icon" /></a>
        </div>
      ...
      <section className="advantage section">
        <div className="advantage-panel">
          <div className="advantage-visual" aria-hidden="true">
            ...
            <div className="advantage-core"><img className="brand-logo" src="/aeyeot-logo.svg" alt="aeyeot logo"/><b>DC</b></div>
          ...
          <div className="advantage-content">
            ...
            <div className="advantage-points">
              <span><Database {...accentIconProps} className="point-icon" /> 彈性整合多種資料來源</span>
              <span><SlidersHorizontal {...accentIconProps} className="point-icon" /> 告警邏輯集中管理</span>
              <span><StackSimple {...accentIconProps} className="point-icon" /> 依營運需求持續擴充</span>
              <span><Cloud {...accentIconProps} className="point-icon" /> 支援雲端與地端部署情境</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="case-studies section" id="cases">
        <div className="section-heading">
          <div><div className="section-kicker">PROVEN IN THE FIELD <span>06</span></div><h2>從現場感測到管理決策，<br />看見不同產業的實用案例。</h2></div>
          <p>每個案例都從真實設備、製程與管理需求出發，將原本分散的資料轉為可監控、可追溯、可行動的資訊。</p>
        </div>
        <div className="case-controls" aria-label="案例輪播控制">
          <div className="case-progress" aria-hidden="true">
            {caseStudies.map((item, index) => (
              <span
                className={index === activeCaseIndex ? "case-progress-dot is-active" : "case-progress-dot"}
                key={item.number}
              />
            ))}
          </div>
          <div className="case-nav">
            <button type="button" className="case-nav-button" aria-label="上一個案例" onClick={() => caseControlRef.current?.nudge(-1)}>
              <CaretLeft {...neutralIconProps} />
            </button>
            <button type="button" className="case-nav-button" aria-label="下一個案例" onClick={() => caseControlRef.current?.nudge(1)}>
              <CaretRight {...neutralIconProps} />
            </button>
          </div>
        </div>
        <div className="case-viewport" ref={caseViewportRef}>
          <div className="case-focus-glow" aria-hidden="true" />
          <div className="case-track" ref={caseTrackRef}>
            {loopedCaseStudies.map((item, index) => (
            <article
              className={index % caseStudies.length === activeCaseIndex ? "case-card is-active" : "case-card"}
            ...
            >
              ...
              <div className="case-body">
                ...
                <h3>{item.title}</h3>
                <p>{item.text}</p>
                <div className="case-result"><CheckCircle {...accentIconProps} className="case-result-icon" />{item.result}</div>
              </div>
            </article>
          ))}
          </div>
      ...
      <section className="scenarios section" id="scenarios">
        <div className="section-heading">
          <div><div className="section-kicker">ONE PLATFORM, MANY INDUSTRIES <span>07</span></div><h2>跨設備、跨場域，<br />把現場資料變成管理能力。</h2></div>
        ...
        <div className="industry-grid">
          {industries.map(([title, text, Icon], index) => (
            <article key={title}>
              ...
              <div className="industry-signal" aria-hidden="true"><Icon {...accentIconProps} className="industry-icon" /></div>
              <h3>{title}</h3>
              <p>{text}</p>
              <span className="industry-arrow"><ArrowUpRight {...accentIconProps} /></span>
            </article>
          ))}
        </div>
      </section>
        ...
        <div className="contact-panel">
          <div className="contact-copy">
            ...
            <div className="contact-meta">
              <span><MagnifyingGlass {...accentIconProps} className="contact-icon" /> 了解現況與需求</span>
              <span><Presentation {...accentIconProps} className="contact-icon" /> 規劃展示與應用情境</span>
              <span><ChatCenteredText {...accentIconProps} className="contact-icon" /> 提供導入方向建議</span>
            </div>
          </div>
          <div className="form-wrap">
            {submitted ? (
              <div className="success-state" role="status">
                <div className="success-icon"><CheckCircle {...accentIconProps} /></div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button className="submit-button" type="submit">送出預約需求 <ArrowUpRight {...neutralIconProps} className="button-icon" /></button>
      ...
      <footer>
        <a className="brand" href="#top" aria-label="AEYEOT Datacanvas 首頁"><img className="brand-logo" src="/aeyeot-logo.svg" alt="AEYEOT Logo" /><span>Datacanvas</span></a>
